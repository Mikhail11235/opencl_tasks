...
kernel void matrix_times_matrix(global float* a,
    ...
    for (int i = a_start, j = b_start; i <= a_end; i += a_step, j += b_step) {
        block_A[local_id_0 + local_id_1 * local_size] = a[i + n * local_id_1 + local_id_0];
        block_B[local_id_0 + local_id_1 * local_size] = b[j + n * local_id_1 + local_id_0];
        barrier(CLK_LOCAL_MEM_FENCE);
        for (int k = 0; k < local_size; ++k) {
            sum += block_A[k + local_id_1 * local_size] * block_B[local_id_0 + k * local_size];
        }
        barrier(CLK_LOCAL_MEM_FENCE);
    }
    result[get_global_id(1) * get_global_size(0) + get_global_id(0)] = sum;
}
)";
